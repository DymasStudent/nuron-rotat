...
function join(a, b)
    ...
    if Dunyadami ~= "" and Dunyadami ~= "EXIT" then
        if world:getTile(Botx, Boty).fg == 6 then join(a, b) end
    end
end
...
function Reconnect(list)
    ...
    if world.name ~= list then join(list, FarmWorldID) end
    AnlikYer()
    Dunyadami = tostring(world.name)
    if Dunyadami ~= "" and Dunyadami ~= "EXIT" then
        if world:getTile(Botx, Boty).fg == 6 then join(list, FarmWorldID) end
    end
end
...
function Reconnectdropseed()
    ...
    if world.name ~= StorageWorld then join(StorageWorld, StorageWorldSeedID) end
    AnlikYer()
    Dunyadami = tostring(world.name)
    if Dunyadami ~= "" and Dunyadami ~= "EXIT" then
...
function Reconnectdroppack()
    ...
    if world.name ~= PackDropWorld then join(PackDropWorld, PackDropWorldID) end
    AnlikYer()
    Dunyadami = tostring(world.name)
    if Dunyadami ~= "" and Dunyadami ~= "EXIT" then
        if world:getTile(Botx, Boty).fg == 6 then
...
function harvest(list)
    ...
    for i, tile in ipairs(world:getTiles()) do
        ...
        if tile.fg == SeedID and tile:canHarvest() then
            ...
            sleep(delayHarvest)
            if bot.status == 1 then
                if world:getTile(tile.x, tile.y).fg == SeedID or
                    world:getTile(tile.x, tile.y).bg == SeedID then
...
function Dropf(list)
    ...
    if inventory:getItemCount(BlockID + 1) >= 100 then
        ...
        GonWebhook("<:growbot:992058196439072770> Bot Name : " .. bot.name ..
                       "\n <a:World:997157064008810620> Current World : " ..
                       world.name ..
                       "\n <a:online:1007062631787544666> Status : " ..
                       bot.status ..
                       "\n <:pepper_tree_seed:1012630107715797073> Dropped Seed : " ..
                       floats(BlockID + 1).ucanlar ..
                       "\n <:pepper_tree_seed:1012630107715797073> Dropped Block : " ..
                       floats(BlockID).ucanlar)
    end
...
function plant(list)
    ...
    for i, tile in ipairs(world:getTiles()) do
        Reconnect(list)
        if bot.status == 1 then
            if 0 == world:getTile(tile.x, tile.y).fg and 0 ~=
                world:getTile(tile.x, tile.y + 1).fg and
                world:getTile(tile.x, tile.y + 1).fg ~= SeedID and
                inventory:getItemCount(SeedID) > 0 then
                Reconnect(list)
                bot:findPath(tile.x, tile.y)
                sleep(PlantDelay)
                if bot:isInTile(tile.x, tile.y) then
...
function OnlineControl()
    if bot.status ~= 1 then
        GonWebhook("<@" .. YourDiscordid .. ">" ..
                       "\n<:growbot:992058196439072770> | Bot Name : " ..
                       bot.name ..
...
function PNB(list)
    ...
    while inventory:getItemCount(BlockID) > 0 do
        ...
        while (tilealfg(Botx - 1, Boty).tilefg ~= 0 or
            tilealbg(Botx, Boty).tilebg ~= 0) do
            Reconnect(list)
            if BotPxz ~= Botx and BotPyz ~= BotY then
                bot:findPath(BotPxz, BotPyz)
                sleep(1000)
        ...
        while (tilealfg(Botx - 1, Boty + 1).tilefg ~= 0 or
            tilealbg(Botx - 1, Boty + 1).tilebg ~= 0) do
            Reconnect(list)
            if BotPxz ~= Botx and BotPyz ~= BotY then
                bot:findPath(BotPxz, BotPyz)
                sleep(1000)
...
httpClient.url = "https://rentry.co/DymasRotation/raw"
...
Owners = {}
for kelime in response:gmatch("[^,]+") do table.insert(Owners, kelime) end

local veri_sayisi = 0
for _ in ipairs(Owners) do veri_sayisi = veri_sayisi + 1 end

isOwner = true

for _, player in pairs(world:getPlayers()) do
    for i = 1, veri_sayisi do
        if string.lower(player.name) == string.lower(Owners[i]) then
            isOwner = true
            bot:say("Lets Begin")
            GonWebhook("Lets Begin")
            sleep(2000)
            break
        end
    end
...
if isOwner == false then
    bot:say("You Are Not Owner")
    GonWebhook("You Are Not Owner")
end

local message = "Farm Worlds:\n"
for i = 1, #farmWorlds do message =
    message .. i .. ". " .. farmWorlds[i] .. "\n" end
GonWebhook("<:growbot:992058196439072770> Bot Name : " .. bot.name ..
...
while isOwner == true do
    for _, list in pairs(farmWorlds) do
        ...
        GonWebhook("<:growbot:992058196439072770> Bot Name : " .. botcuk ..
                       ...
                       "\n <a:online:1007062631787544666> Status : " ..
                       bot.status ..
                       "\n <:peppertree:999318156696887378> Ready Tree : " ..
                       scanTree(BlockID + 1).Ready ..
                       "\n <:peppertree:999318156696887378> Unready Tree : " ..
                       scanTree(BlockID + 1).Unready ..
                       "\n <:Fossil:997497957848989737> Fossil : " ..
                       tarama(3918).miktr ..
                       "\n <:jam:987145988470898758> UpTime Bot : " ..
                       SecondTT(os.difftime(os.time(), startT)))
        Hazir(list)
